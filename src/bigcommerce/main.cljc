(ns bigcommerce.main
  "Kotodama WASM entrypoint for the Bigcommerce clean-room actor (L4) — Clojure port.
  ...
  facts (`bigcommerce.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "bigcommerce")
(def tier "L4")
...
(def entity-specs
  [{:entity "Product"  :plural "products"  :id-prefix "bigcomme_pro"
    :fields [:name :type :sku :description :price :availability :condition :inventoryTracking :isVisible :dateCreated]
    :required [:name]
    :coerce {:price :float}
    :refs {}}
   {:entity "Order"    :plural "orders"    :id-prefix "bigcomme_ord"
    :fields [:customerId :email :status :total :subtotal :currencyCode :paymentMethod :dateCreated]
    :required [:email]
    :coerce {:customerId :int :total :float :subtotal :float}
    :refs {:customerId "Customer"}}
   {:entity "Customer" :plural "customers" :id-prefix "bigcomme_cus"
    :fields [:email :firstName :lastName :company :phone :customerGroupId :dateCreated]
    :required [:email]
    :coerce {:customerGroupId :int}
    :refs {}}
   {:entity "Category" :plural "categories" :id-prefix "bigcomme_cat"
    :fields [:name :parentId :description :isVisible :sortOrder :defaultProductSort]
    :required [:name]
    :coerce {:parentId :int :isVisible :bool :sortOrder :int}
    :refs {}}
   {:entity "Brand"    :plural "brands"    :id-prefix "bigcomme_bra"
    :fields [:name :pageTitle :searchKeywords :imageUrl]
    :required [:name]
    :coerce {}
    :refs {}}
   {:entity "Cart"     :plural "carts"     :id-prefix "bigcomme_car"
    :fields [:customerId :email :channelId :baseAmount :cartAmount :createdTime]
    :required [:email]
    :coerce {:customerId :int :channelId :int :baseAmount :float :cartAmount :float}
    :refs {:customerId "Customer"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"bigcommerce.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "bigcommerce-compat" :tier tier :entities entities} 200])
...
(defn start! [] :bigcommerce-compat/ready)
